	...
	for (int i = y / 32; i < (y + h) / 32; i++)//проходимся по элементам карты
		for (int j = x / 32; j<(x + w) / 32; j++)
		{
			...
			if (Dx < 0) { x = j * 32 + 32; dx = 0; }// с левым краем карты
				...
				if (TileMap[i][j] == 'f') {
					health -= 30;//если взяли ядовитый цветок								
					TileMap[i][j] = ' ';//убрали цветок
				}

				if (TileMap[i][j] == 'h') {
					health += 10;//если взяли сердечко
...
public:
	...
	if (name == "EasyEnemy"){//Задаем спрайту один прямоугольник для вывода одного игрока. IntRect – для приведения типов
		sprite.setTextureRect(IntRect(0, 0, w, h));
